...
export function BountyBoard({ isSidebar = false }) {
  ...
  return (
    <div className={`${isSidebar ? 'bg-transparent border-none p-0 shadow-none' : 'bg-bg-card border border-gray-800 rounded-md p-xl shadow-card'} relative overflow-hidden`}>
      ...
      <div className={`flex justify-between items-center ${isCollapsed ? 'mb-0' : 'mb-lg'} relative z-10 cursor-pointer`} onClick={() => setIsCollapsed(!isCollapsed)}>
        <div className="flex items-center gap-md">
          <Target size={isSidebar ? 16 : 20} className="text-primary shadow-primary-glow" />
          <h2 className={`font-display ${isSidebar ? 'text-[0.7rem]' : 'text-[1rem]'} font-black uppercase tracking-[0.2em] m-0 text-white`}>Bounties</h2>
          ...
          >
            <div className={`grid ${isSidebar ? 'gap-sm' : 'gap-md'} relative z-10`}>
              {missions.map((mission) => (
                <motion.div
                  key={mission.id}
                  className={`group flex items-center ${isSidebar ? 'gap-sm p-sm' : 'gap-lg p-lg'} rounded-sm border transition-all duration-fast ${
                    mission.completed 
